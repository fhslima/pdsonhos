...
function renderMap() {
  ...
  content.innerHTML = `<div class="map-cover"><div><h3>TOMATe</h3><p>Do sonho à agenda</p></div><span class="map-date">Criado em ${date}</span></div>${selected.length ? `<div class="map-themes">${selected.map(id => `<span>${parseId(id).theme}</span>`).join("")}</div><div class="map-objectives">${selected.map(id => { const item=parseId(id), objective=state.objectives[id]?.trim(); return `<div class="map-item"><strong>${item.theme}<br><small>${item.group} · ${item.field}</small></strong><p class="${objective ? "" : "no-objective"}">${objective || "Objetivo ainda não definido."}</p></div>`; }).join("")}</div>` : `<div class="empty-state"><span>◇</span><h3>Seu mapa está esperando</h3><p>Selecione temas e escreva seus objetivos para construir esta visão.</p><button class="secondary next" data-next="temas">Começar agora</button></div>`}`;
}
